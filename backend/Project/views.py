...
class ShowProjects(APIView):
    ...
    def get(self, request):
        try:
            ...
            projects = Project.objects.filter(admin_confirmed=True, is_full=False, is_delete=False).order_by('-created_at')[lastShow: lastShow + 9]
...
class ShowSingleProject(APIView):
    ...
    def get(self, request):
        try:
            ...
            if not Project.objects.filter(id=id, admin_confirmed=True, is_delete=False).exists():
            ...
            if project.is_publish and AcceptedProject.objects.filter(project=project, freelance__account__user=request.user).exist():
                pass    # SHOW FULL DESCROPTION AND FILES
            else:
                pass 
            ...
            data = [{
                ...
                'files': [x.url for x in project.files] if project.is_publish else None, # 
